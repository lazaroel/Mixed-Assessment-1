...
author: "Chief Science Officer Elric Francis Lazaro; ID: 1003457644"
output:
...
header-includes:    
  ...
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[LO, LE]{Elric Francis Lazaro, 1003457644}
...
```{r}
crew_data$name[crew_data$position == "Communications Officer"][1]
```

3. How many crewmembers are in this dataset? 

```{r}
length(unique(crew_data$crew_id))
```
...
```{r}
## Check all possible positions and check for mispelling
...
complete_engineers <- crew_data %>% 
  filter(sub_division == "Engineering")
engineers_pos <- complete_engineers %>% 
  filter(str_detect(str_to_lower(position), "engineer") | str_detect(str_to_lower(position), "engineering"))

length(unique(engineers_pos$crew_id))/length(unique(complete_engineers$crew_id))
```
About 62.07% of people in Engineering subdivision have 'engineer' or 'engineering' in their position title
...
```{r}
week1 %>% 
  ggplot(aes(x = duty_shift, y = productivity, color = shift_team)) +
  geom_boxplot() +
...
w1_team <- lm(productivity ~ full_team, data = week1)
...
```
We have very strong evidence against the hypothesis that the simpler model (1a) fits the data just as well - choose 1b
...
```
i) We have very strong evidence against the hypothesis that the simpler model (1b) fits the data just as well - choose 2a
ii) The two models are the same as interaction between duty shift and shift team is the same as full team given it is a concatenation of the two - choose 2b
...
```
 We don't have strong evidence against the hypothesis that the simpler model (2b) fits the data just as well - choose 2b over 3
...
```{r}
incomplete_comp <- competition_data %>%
  pivot_longer(-c(crewmember), 
               names_to = "plant", values_to = "num_recorded") %>% 
...
complete_comp <- competition_data %>%
  pivot_longer(-c(crewmember), 
               names_to = "plant", values_to = "num_recorded") %>% 
  right_join(origin_data, by = "plant") %>% 
  filter(!(is.na(num_recorded))) %>% 
  distinct(crewmember, native_to) %>% # this line will achieve instruction 5
  group_by(crewmember) %>% # these last two lines achieve instruction 6
